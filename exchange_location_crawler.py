...
def get_exchange_loacation_information():
    ...
    keywords = ["환전소", "외환", "환전", "외화 환전", "환전 가능한 은행","국민은행","신한은행","우리은행"]
    ...
    filter = ["식품","연구원","출장소","문화의집","ATM","365","충전소","주차장","연수원","대리점"]
    df = df[~df["place_name"].str.contains("|".join(filter),na=False)]
    df['address_name'] = df['address_name'].apply(lambda x: x.replace("특별자치도",""))
    df['address_name'] = df['address_name'].apply(lambda x: x.replace("특별자치시",""))
    return df
